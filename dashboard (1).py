...
try:
    ...
    for _, row in employees_df.iterrows():
        if pd.notna(row['9-Box_Placement']):
            ...
            matrix_data.at[pot.replace(" Potential", ""), perf] = matrix_data.at[pot.replace(" Potential", ""), perf] + 1 if pd.notna(matrix_data.at[pot.replace(" Potential", ""), perf]) else 1
            matrix_data = matrix_data.fillna(0).astype(int).infer_objects(copy=False)  # Fix for FutureWarning
